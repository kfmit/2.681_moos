...
bool CFrontEstimate::OnStartUp()
{
  ...
  if(m_MissionReader.GetConfigurationParam("adaptive",sVal))
   ...
   MOOSTrace("RIGHT BEFORE COOLINGSTEPS \n");
  delta_t = 1/(cooling_steps);

  // Initialize annealer
  MOOSTrace("RIGHT BEFORE INITIAL \n");
   MOOSTrace("RIGHT BEFORE INITIAL \n");
    MOOSTrace("RIGHT BEFORE INITIAL \n");
     MOOSTrace("RIGHT BEFORE INITIAL \n");
     MOOSTrace("RIGHT BEFORE INITIAL \n");
...
bool CFrontEstimate::OnConnectToServer()
{
  ...
  Register("DONE_WITH_FIRST",0);
  Register("GIVE_ME_UPDATE",0);
...
// This loop checks ALL messages
  for(p=NewMail.begin(); p!=NewMail.end(); p++)
      ...
      if (rMsg.m_sKey == "UCTD_MSMNT_REPORT" && in_survey)
	{
	  value = rMsg.m_sVal;
          string value1=value;
          string b=tokStringParse(value1, "vname", ',', '=');
          if(b!=vname){
            string left = biteString(value, ',');
               stringstream my_s;
               my_s<<"vname="<<vname<<","<<value;
               string my_fix=my_s.str();
               value=my_fix;
          }
	  CMeasurement buf;
	  ...
	  MOOSTrace("New measurement added, Total = %d\n", num_meas);
      ...
      else if (rMsg.m_sKey == "DONE_WITH_FIRST"){
        if(rMsg.m_sVal =="true"){
          stringstream my_note_string;
          my_note_string<<"DONE_WITH_FIRST_"<<vname;
          ...
          node_message.setSourceNode(vname);
          node_message.setDestNode("all");
          node_message.setVarName(note);
          node_message.setStringVal("true");
       ...
       else if (rMsg.m_sKey == "GIVE_ME_UPDATE"){
         if(rMsg.m_sVal =="YEP"){
           Notify("INSIDE","Before");
           vector<double> result;
           anneal.getEstimate(result);
           offset =     result[0];
           angle  =     result[1];
